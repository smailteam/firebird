...
app.setWindowIcon(QtGui.QIcon(os.path.abspath(__file__).rstrip('__main__.py')+'talonflame.webp'))
widget=QWidget()
...
title.setFont(QFont('Verdana',12))
...
def login():
	def login():
		global raw
		try:
			raw=mail.RawMail('http://'+e1.text().split('@')[1],e1.text(),e2.text(),verify=False)
			conn.execute('INSERT INTO users VALUES("http://'+e1.text().split('@')[1]+'","'+e1.text()+'","'+e2.text()+'")')
			conn.commit()
